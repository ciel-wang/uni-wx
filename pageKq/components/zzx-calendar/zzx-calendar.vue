	...
	<view class="zzx-calendar">
		...
		<view class="calendar-content">
		   <swiper 
				...
				:style="{ width: '100%', height: sheight }" 
				...
				<swiper-item class="calendar-item" v-for="sitem in swiper" :key="sitem">
					<view class="calendar-days">
						...
						<template v-if="sitem === current">
							<view 
								class="calendar-day" v-for="(item,index) in days" :key="index"
								:class="{
								...
								<view
									...
									:class="[
										...
										item.info && item.info.dayMode == 2 ? dayModeClass : ''
									]"
								>
									{{item.time.getDate()}}
								</view>
								<view class="dot-show" v-if="item.info" :style="dotStyle">{{item.info.classTimes}}</view>
							</view>
						</template>
						...
						<template v-else>
							...
							<template v-if="current - sitem === 1 || current-sitem ===-2">
								<view 
									class="calendar-day" 
									v-for="(item,index) in predays" :key="index"
									:class="{
										'day-hidden': !item.show
									}">
									<view
			...
			<!-- 周和月切换部门，打开或折叠 -->
			<view @click="changeMode" style="width: 100%;display: flex;justify-content: space-around;align-items: center;margin-bottom: 10rpx;">
				<text style="width: 40%;height: 2rpx;background-color: #999;display: inline-block;"></text>
				<u-icon name="arrow-down" v-if="weekMode"></u-icon>
